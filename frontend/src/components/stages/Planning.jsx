...
export const Planning = ({ topicId, onComplete }) => {
  ...
  const fetchChapters = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/dashboard/chapters/${topicId}`);
      if (res.ok) {
  ...
  const fetchWorkflowStatus = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/dashboard/workflow-status/${topicId}`);
      if (res.ok) {
        const data = await res.json();
  ...
  const startPlanning = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/agents/planner`, {
        method: 'POST',
    ...
    pollingRef.current = setInterval(async () => {
      try {
        const [chapRes, wfRes] = await Promise.all([
          fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/dashboard/chapters/${topicId}`),
          fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/dashboard/workflow-status/${topicId}`),
        ]);
        if (chapRes.ok) {
