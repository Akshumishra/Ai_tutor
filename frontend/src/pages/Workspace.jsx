...
const CurriculumStage = ({ topicId, userId, onComplete }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const [histRes, curRes] = await Promise.all([
          fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/agents/chat-history?topic_id=${topicId}&agent_type=curriculum`, { signal: controller.signal }),
          fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/dashboard/curriculum/${topicId}`, { signal: controller.signal }),
        ]);
        if (histRes.ok) {
    ...
    (async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/dashboard/workflow-status/${topicId}`, { signal: controller.signal });
        if (res.ok) {
  ...
  useEffect(() => {
    ...
    if (lastMsg?.status === 'generating' && lastMsg.id && !reconnectingRef.current) {
      ...
      fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/agents/reconnect/${lastMsg.id}`)
        .then(res => processStream(res))
        .catch(err => console.error(err))
        .finally(() => {
          reconnectingRef.current = false;
          fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/agents/chat-history?topic_id=${topicId}&agent_type=curriculum`)
            .then(res => res.json())
            .then(history => {
              if (history?.length > 0) {
                setChats(prev => {
  ...
  const handleSend = async (e) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/agents/curriculum`, {
        method: 'POST',
      ...
      try {
        const curRes = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/dashboard/curriculum/${topicId}`);
        if (curRes.ok) { const d = await curRes.json(); if (d?.length > 0) setCanvasContent(d); }
      } catch (error) { console.error(error); }
      try {
        const wfRes = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/dashboard/workflow-status/${topicId}`);
  ...
  const handleNextStep = async () => {
    ...
    if (canvasContent && canvasContent.length > 0) {
      try {
        await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/dashboard/curriculum/${topicId}/sync-chapters`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    if (!isCompleted) {
      try {
        await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/dashboard/curriculum/${topicId}/finalize`, { method: 'POST' });
...
export const Workspace = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/dashboard/topics/create`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    (async () => {
      ...
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/dashboard/workflow-status/${topicId}`,
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/dashboard/topics/${topicId}/time`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
