...
async def arun_curriculum_agent(user_id: str, topic_id: str, chat_history: list[dict], user_input):
    ...
    try:
        async for event in agent.arun(chat_history=chat_history,user_input=user_input):
            yield event
...
def run_planner_agent(topic_id: str):
    ...
    try:
        ...
        ws = db.query(WorkflowStatus).filter(
            ...
            WorkflowStatus.stage    == WorkflowStage.PLANNING.value,
        ).first()
        if not ws:
            ws = WorkflowStatus(topic_id=UUID(topic_id), stage=WorkflowStage.PLANNING.value)
            db.add(ws)
...
async def arun_teacher_agent(chapter_id, chat_history, user_message):
    agent = TeacherAgent(
    ...
    try:
        async for event in agent.arun(chat_history=chat_history, user_message=user_message):
            yield event
    except Exception as e:
        raise e